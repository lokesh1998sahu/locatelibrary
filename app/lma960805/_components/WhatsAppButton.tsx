...
export default function WhatsAppButton({ phones, className, label, text }:{
  ...
  return (
    <div ref={wrap} className="relative shrink-0">
      <button type="button" onClick={onClick} disabled={list.length===0} className={className||"px-2.5 py-2.5 rounded-xl bg-lma-accent/10 text-lma-accent font-bold text-xs disabled:opacity-40"}>{label||"💬"}{multi?" ▾":""}</button>
      {open && multi && (
        <div className="fixed inset-0 z-[10002] flex items-center justify-center px-8" onClick={()=>setOpen(false)}>
          ...
          <div className="relative w-full max-w-xs bg-white rounded-2xl overflow-hidden shadow-xl lma-slide-up" onClick={e=>e.stopPropagation()}>
            <div className="px-4 py-3 bg-lma-accent/10 border-b border-lma-accent/20 flex items-center">
              <span className="text-sm font-extrabold text-lma-accent">WhatsApp chat</span>
              <span className="ml-auto text-[11px] font-bold text-lma-slate-400">{list.length} numbers</span>
            </div>
            {list.map((l,i)=>(
