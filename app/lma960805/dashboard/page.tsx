      ...
      {customOpen&&(
        <div className="bg-white rounded-xl p-3 mb-3 shadow-sm flex items-end gap-2 lma-slide-up">
          <label className="flex-1 text-[11px] font-bold text-lma-slate-500">From
            <input type="date" value={`${from.getFullYear()}-${pad(from.getMonth()+1)}-${pad(from.getDate())}`} onChange={e=>{const d=new Date(e.target.value);if(!isNaN(d.getTime()))setFrom(d);}} className="w-full mt-1 px-2 py-2 rounded-lg border-[1.5px] border-lma-slate-200 text-sm font-medium text-lma-slate-800"/><span className="block text-[10px] font-bold text-lma-slate-500 mt-1">{fmtDMY(`${from.getFullYear()}-${pad(from.getMonth()+1)}-${pad(from.getDate())}`)}</span>
          </label>
          <label className="flex-1 text-[11px] font-bold text-lma-slate-500">To
            <input type="date" value={`${to.getFullYear()}-${pad(to.getMonth()+1)}-${pad(to.getDate())}`} onChange={e=>{const d=new Date(e.target.value);if(!isNaN(d.getTime()))setTo(d);}} className="w-full mt-1 px-2 py-2 rounded-lg border-[1.5px] border-lma-slate-200 text-sm font-medium text-lma-slate-800"/><span className="block text-[10px] font-bold text-lma-slate-500 mt-1">{fmtDMY(`${to.getFullYear()}-${pad(to.getMonth()+1)}-${pad(to.getDate())}`)}</span>
          </label>
