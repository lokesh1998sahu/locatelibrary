      ...
      {loading&&expiring.length===0&&expired.length===0&&cancellations.length===0?(
        ...
        <>
          {expiring.length===0&&expired.length===0?(
            ...
            <>
              {expired.length>0&&(
                <Group title="Expired" tone="danger" count={expired.length}>
                  {expired.map(it=><QueueCard key={it.receipt_no} it={it} router={router} onCancel={()=>setActionFor(it)} onDoNotRenew={()=>doDoNotRenew(it)}/>)}
                </Group>
              )}
              {expiring.length>0&&(
                <Group title="Expiring soon" tone="warn" count={expiring.length}>
                  {expiring.map(it=><QueueCard key={it.receipt_no} it={it} router={router} onCancel={()=>setActionFor(it)} onDoNotRenew={()=>doDoNotRenew(it)}/>)}
                </Group>
              )}
            </>
          )}
              ...
              {cancellations.map(it=>(
                <div key={it.receipt_no} className="bg-white rounded-xl p-3 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2.5">
                    ...
                    <button onClick={()=>doReset(it)} className="py-2 rounded-lg bg-lma-slate-100 text-lma-slate-600 font-bold text-xs">Undo (Reset)</button>
                  </div>
                </div>
              ))}
            </div>
...
function Group({ title, tone, count, children }:{ title:string; tone:"danger"|"warn"; count:number; children:React.ReactNode }){
  const cls=tone==="danger"?"text-lma-danger":"text-lma-warn";
  return (
    <div className="mb-4">
  ...
  return (
    <div className="bg-white rounded-xl p-3 shadow-sm">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className={`text-[10px] font-bold ml-auto ${it.days_until_expiry<0?"text-lma-danger":"text-lma-warn"}`}>{daysLabel}</span>
      </div>
      <div className="text-sm font-semibold text-lma-slate-800 truncate">{it.name}</div>
